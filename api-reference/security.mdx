...
In addition to cryptography, our API layer implements several protective measures:

### IP Allowlisting
Restrict API access strictly to your backend servers. By defining an IP Allowlist in your dashboard, OTP Edge will instantly reject any request originating from an unknown IP address, neutralizing the threat of stolen API keys.
...
All public endpoints are guarded by strict, per-workspace rate limiters (e.g., 60 requests/minute for OTP dispatches). This prevents brute-force attacks and shields your Meta API quotas from being drained by malicious bots. Note: Bulk Marketing Campaigns initiated via the dashboard deliberately bypass these limits, ensuring your marketing scales without restriction.
